...
object HelloWorldBatchApp {
  ...
  def main(args: Array[String]): Unit = {
    try {
      ...
      // This initiates spark. Without this, we can't process data so easily or use spark at all.
      val spark = SparkSession.builder()
        .appName(jobName)
        .config("spark.sql.shuffle.partitions", "3")
        //TODO- What is local[*] doing here?
        // This is telling the spark session that we are using all cores on the machine, specifically all cores on our local computer
        .master("local[*]")
